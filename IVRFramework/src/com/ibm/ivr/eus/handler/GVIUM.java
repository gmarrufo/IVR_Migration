 ...
 * @author MAYMITRA
 */
public class GVIUM extends HttpServlet implements Servlet {

	private static final long serialVersionUID = 1L;
	private static Logger LOGGER = Logger.getLogger(GVIUM.class);
    ...
    throws ServletException, IOException {
		...
		if (testCall)
			LOGGER.debug(new StringBuffer(logToken).append("Entering GVIUM Handler"));

        String filename = globalProp.getProperty("gviUMFile");// property value as in xml file
        String phone = (String) session.getAttribute("phone");
        
        String route = GviHelper.getGviLocation(filename, phone, testCall, logToken);
        
        if (route != null && route.equalsIgnoreCase("UM")) {
        	if (testCall)
    			LOGGER.debug(new StringBuffer(logToken).append("Found GVIUM route"));
        	session.setAttribute("xfer_var", route);
        	session.setAttribute("gvi_code", route);
        } else {
        	if (testCall)
    			LOGGER.debug(new StringBuffer(logToken).append("Did not find GVIUM route, trying GVIROUTE"));
        	filename = globalProp.getProperty("gviRouteFile");
        	route = GviHelper.getGviLocation(filename, phone, testCall, logToken);
        	session.setAttribute("xfer_var", route);
        	session.setAttribute("gvi_code", route);
        }
		...
		if (testCall) {
			...
			LOGGER.debug(new StringBuffer(logToken).append("Exiting GVIUM handler"));
		}
		return;
     }
