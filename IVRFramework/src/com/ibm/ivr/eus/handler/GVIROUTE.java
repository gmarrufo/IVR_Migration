 ...
 * @author MAYMITRA
 */
public class GVIROUTE extends HttpServlet implements Servlet {

	private static final long serialVersionUID = 1L;
	private static Logger LOGGER = Logger.getLogger(GVIROUTE.class);
    ...
    throws ServletException, IOException {
		...
		if (testCall)
			LOGGER.debug(new StringBuffer(logToken).append("Entering GVIROUTE Handler"));

        String filename = globalProp.getProperty("gviRouteFile");// property value as in xml file
		...
		if (testCall) {
			...
			LOGGER.debug(new StringBuffer(logToken).append("Exiting GVIROUTE handler"));
		}
		return;
