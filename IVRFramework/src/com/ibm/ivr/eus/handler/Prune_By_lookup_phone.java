 ...
 *
 * Outputs:
 *   - return_vector (pruned of rows that do not match phone)
 *   - return_count (after pruned)
 * 
 ************************************************************************/

public class Prune_By_lookup_phone extends HttpServlet implements Servlet{
	...
	private static Logger LOGGER = Logger.getLogger(Prune_By_lookup_phone.class);
	...
	String logToken = new StringBuffer("[").append(callid).append("] ").toString();
	 ...
	 if (testCall)
	 	LOGGER.debug(new StringBuffer(logToken).append("Entering Prune_By_lookup_phone handler"));
	...
	Integer row_index = 0;
	String row = null;
	String lookup_phone = null;
   	Vector<StringBuffer> valueSet = null;
   	
    try {
        // Get attributes from the request
    	lookup_phone = (String) session.getAttribute("lookup_phone");
    	valueSet = (Vector<StringBuffer>) session.getAttribute("return_vector");

    	return_count = valueSet.size();
        // Debug Logging		
    	if (testCall){
    		LOGGER.debug(new StringBuffer(logToken).append("lookup_phone = "+lookup_phone));	
